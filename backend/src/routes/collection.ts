...
router.put('/:formId', async (req: Request, res: Response) => {
  ...
  try {
    const current = await pool.query(
      'SELECT owned, is_definitive, needs_trade, notes FROM collection_entries WHERE form_id = $1',
      [formId]
    );
    ...
    const { rows } = await pool.query(
      `UPDATE collection_entries
       SET owned = $1, is_definitive = $2, needs_trade = $3, notes = $4, updated_at = now()
       WHERE form_id = $5
       RETURNING form_id, owned, is_definitive, needs_trade, notes, tcg_card_id, updated_at`,
      [normalized.owned, normalized.is_definitive, normalized.needs_trade, notes, formId]
    );
    res.json(rows[0]);
  } catch (err) {
